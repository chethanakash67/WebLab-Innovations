...
export default function Marquee() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-white/[0.08] py-8">
      {/* Fade edges */}
      ...
      <div className="flex gap-5 animate-marquee hover:[animation-play-state:paused]" style={{ width: "max-content" }}>
        {tripled.map((tech, i) => (
          <div
            key={`${tech}-${i}`}
            className="flex h-20 w-64 flex-shrink-0 cursor-default items-center justify-center gap-2.5 rounded-full border border-white/15 bg-black/30 px-7 text-sm font-semibold tracking-wide text-white/70 backdrop-blur-sm transition-all duration-500 hover:border-primary/50 hover:bg-primary/5 hover:text-white"
          >
